...
from .serializers import UserBalanceSerializer, UserSerializer, UserTransactionHistorySerializer
...
class UserTransactionHistoryView(ListAPIView):
  ...
  def get_queryset(self):
    ...
    query = UserTransactionHistory.objects.filter(
      uid = uid
    )
...
class UserTopUpView(APIView):
  ...
  def put(self, request):  
    try:
      ...
      user_balance_instance = UserBalance.objects.get(uid=uid, account_number=account_number)

      with transaction.atomic():
        UserBalance.add_balance(uid, amount)

        instance = UserTransactionHistory(uid=user_instance, account_number=user_balance_instance, amount=amount, description=f'topup {amount}')
        instance.save()
